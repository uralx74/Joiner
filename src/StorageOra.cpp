...
// TStorageOra
//---------------------------------------------------------------------------
// Описание механизма работы с TOraSession и TOraQuery
//  try {
...
void TStorageOraSql::openTable(bool ReadOnly)
{
    ...
    // Если источник, то присваивать значение FieldsCount = dbQuery->Fields->Count
    //FieldCount = dbQuery->Fields->Count;
    // Возможно сделать цикл по OraQuery->Fields и заполнять Fields
    // если  Fields.size() == 0
    FieldCount = Fields.size();
    if (FieldCount == 0) {
        FieldCount = dbQuery->Fields->Count;
        ...
        for (int i = 0; i < dbQuery->Fields->Count; i++) {
            TOraField* field = addField();      // Возможно заменить на
...
//---------------------------------------------------------------------------
// Возвращает признак наличия поля с именем
