            ...
            else if (sImportType == "excel")
            {
                ...
                while ( !VarIsClear(subnode))
                {
                    if (msxml.GetNodeName(subnode) == "excel")
                    {
                        TExcelTable Table;
                        Table.File = ExpandFileNameCustom(msxml.GetAttributeValue(subnode, "file"), clConfigPath);  // Имя файла может быть задано маской
                        Table.WorksheetIndex = msxml.GetAttributeValue(subnode, "worksheet", 1);  // Индекс листа с таблицей
                        Table.TitleRowIndex = msxml.GetAttributeValue(subnode, "first_row", 1);  // Первая строка = заголовок таблицы
                        Table.LastDataRowIndex = msxml.GetAttributeValue(subnode, "last_row", 0);  // Последняя строка = последняя запись
